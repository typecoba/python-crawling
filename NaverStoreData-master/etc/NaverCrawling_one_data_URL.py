...
def getComment(s_name, p_number):
    ...
    url01 = 'http://smartstore.naver.com/'
    url03 = '/products/'
    url05 = '/reviews/page.json?page=1&size=10&sortType=REVIEW_RANKING&contentType=ALL'
    url_all3 = url01 + s_name + url03 + p_number + url05
    ...
    plain_text3 = get_request_url(url_all3)

    mydatas = []
    for i in [plain_text1, plain_text2, plain_text3]:
        if (i == None):
            return None
        else:
...
def main():
    ...
    s_name = str(_s_name.search(url, 28).group())
    p_number = str(_p_number.search(url, 37).group())
